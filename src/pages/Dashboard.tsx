...
const Dashboard: React.FC = () => {
  ...
  const dashboardData = useMemo(() => {
    // Calculate dynamic account balances based on transactions
    const accountBalances = accounts.reduce((acc, account) => {
      // Start with the initial account balance
    ...
    // Calculate total balance from dynamic balances
    const totalBalance = Object.values(accountBalances).reduce(
      (sum: number, account: any) => sum + account.currentBalance,
      0
    );
    ...
    const categoryData = Object.entries(categoryExpenses).map(
      ([category, amount]) => ({
        name: category,
        value: amount,
      })
    ...
    return {
      ...
      investmentGainLoss,
      accountBalances: Object.values(accountBalances), // Include dynamic balances for display
    };
    ...
    <div className="space-y-6 lg:space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
        ...
        <motion.div
          ...
          className="bg-white p-4 lg:p-6 rounded-2xl shadow-sm border"
        >
          <h3 className="text-lg font-bold text-gray-900 mb-4 lg:mb-6">
      ...
      >
        ...
        {dashboardData.accountBalances &&
        dashboardData.accountBalances.length > 0 ? (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {account.balance !== account.currentBalance && (
                      <p className="text-xs text-gray-500">
                        Initial: ₹{account.balance.toLocaleString()}
                      </p>
                    )}
                  </div>
          ...
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-gray-600 mb-6">
              Add your first bank account to get started with tracking your
              finances.
            </p>
            <Link
              to="/settings"
