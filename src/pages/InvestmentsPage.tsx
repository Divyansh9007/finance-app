...
const schema = yup.object({
  ...
  type: yup.string().oneOf(['stock', 'mutual_fund', 'crypto', 'gold', 'sip', 'other']).required('Type is required'),
  quantity: yup.number().positive('Quantity must be positive').required('Quantity is required'),
  buyPrice: yup.number().positive('Buy price must be positive').required('Buy price is required'),
  currentPrice: yup.number().positive('Current price must be positive').required('Current price is required')
});
...
const InvestmentsPage: React.FC = () => {
  ...
  const portfolioData = React.useMemo(() => {
    ...
    const totalGainLossPercentage = totalInvestmentCost > 0 
      ? (totalGainLoss / totalInvestmentCost) * 100 
      : 0;
    ...
    const typeData = Object.values(typeBreakdown).map((type: any) => ({
      ...type,
      name: INVESTMENT_TYPES.find(t => t.value === type.type)?.label || type.type,
      percentage: totalInvestmentValue > 0 ? (type.value / totalInvestmentValue) * 100 : 0
    }));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">Track your stocks, mutual funds, and other investments</p>
        </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-2xl font-bold ${
                portfolioData.totalGainLossPercentage >= 0 ? 'text-green-600' : 'text-red-600'
              }`}>
      ...
      {portfolioData.typeData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <motion.div
            ...
            className="bg-white p-6 rounded-2xl shadow-sm border"
          >
            <h3 className="text-lg font-bold text-gray-900 mb-4">Type-wise Performance</h3>
            <ResponsiveContainer width="100%" height={300}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                      investment.gainLoss >= 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'
                    }`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-600">
                        <span>{INVESTMENT_TYPES.find(t => t.value === investment.type)?.label}</span>
                        <span>•</span>
